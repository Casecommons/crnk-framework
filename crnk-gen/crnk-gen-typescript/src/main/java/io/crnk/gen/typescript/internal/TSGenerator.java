...
public class TSGenerator {
	...
	public void run() throws IOException {
		...
		} else if (config.getFormat() == TSResourceFormat.NGRX) {
			NgrxJsonApiLibrary.initNgrx();
			CrnkLibrary.initNgrx(config.getNpm().getPackageName());
		} else {
			NgrxJsonApiLibrary.initNgrx();
			CrnkLibrary.initCrnk();
		}
	...
	protected void writeSources() throws IOException {
		...
		if (config.getFormat() == TSResourceFormat.PLAINJSON) {
			writeBaseTs("crnk-plain.ts");
		} else if (config.getFormat() == TSResourceFormat.NGRX) {
			writeBaseTs("crnk-ngrx.ts");
		}
	}

	private void writeBaseTs(String name) throws IOException {
